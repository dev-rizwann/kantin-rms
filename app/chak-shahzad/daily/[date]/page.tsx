...
import { DayOrdersLazy } from "@/app/h8/daily/DayOrdersLazy"

export const dynamic = "force-dynamic"
const SLUG = "chak-shahzad"

export default async function ChakShahzadDayDetail({ params }: { params: { date: string } }) {
  const date = params.date
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) notFound()
  const d = await getH8DayDetailLive(date, SLUG)
  const k = d.kpis
  const maxCat = Math.max(1, ...d.categories.map((c) => c.sales))
  const maxItem = Math.max(1, ...d.items.map((i) => i.sales))

  const back = <Link href={`/${SLUG}/daily`} className="text-[13px] font-medium text-coral-700 hover:underline">← Daily &amp; Cash</Link>
  ...
  if (!d.hasData) {
    return (
      <>
        ...
        <div className="rounded-xl border border-stone-200 bg-white px-4 py-10 text-center text-sm text-stone-500">No sales recorded on this date.</div>
      </>
    )
  }
    ...
    <>
      ...
      <div className="mb-6 grid gap-4 lg:grid-cols-3">
        ...
        <Card>
          ...
          <CardBody>
            <LedgerTable rows={d.cashiers} cols={[
      ...
      <section className="mb-6">
        ...
        <LedgerTable rows={d.categories} rank cols={[
      ...
      <section className="mb-6">
        ...
        <LedgerTable rows={d.items} rank cols={[
      ...
      <section className="mb-6">
        ...
        <DayOrdersLazy date={date} slug={SLUG} />
      </section>

      <section>
        <SectionHead title="Payments" context="net collected by type" />
        <LedgerTable rows={d.payments} cols={[
