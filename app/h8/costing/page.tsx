  ...
  return <>
    ...
    <section className="mb-7">
      ...
      <div className="overflow-x-auto rounded-xl border border-stone-200 bg-white shadow-[0_1px_2px_rgba(28,25,23,0.04)]">
        <table className="w-full text-[12px]"><thead><tr className="border-b border-stone-200 bg-stone-50/80 text-[10.5px] uppercase tracking-wide text-stone-400">
          <th className="px-3 py-2 text-left">Recipe</th><th className="px-3 py-2 text-right">Plate cost</th><th className="px-3 py-2 text-right">Sell price</th><th className="px-3 py-2 text-right">FC %</th><th className="px-3 py-2 text-right">Margin</th><th className="px-3 py-2 text-right">Suggested @ target</th><th className="whitespace-nowrap px-3 py-1.5">Status</th>
          ...
          return <tr key={row.id} className="hover:bg-coral-50/60">
            <td className="whitespace-nowrap px-3 py-1.5">{canEdit ? <Link href={`/h8/costing/recipes/${row.id}`} className="font-medium text-stone-900 hover:text-coral-700">{row.name}</Link> : <span className="font-medium text-stone-900">{row.name}</span>}<span className="ml-1.5 text-[10px] text-stone-400">v{row.version}</span></td>
            <td className="whitespace-nowrap px-3 py-1.5 text-right font-medium tabular-nums">{money(row.plateCost)}</td><td className="whitespace-nowrap px-3 py-1.5 text-right tabular-nums">{money(row.sellPrice)}</td>
            <td className="whitespace-nowrap px-3 py-1.5 text-right font-semibold tabular-nums">{pct(row.foodCostPct)}</td><td className={"whitespace-nowrap px-3 py-1.5 text-right tabular-nums " + ((row.margin ?? 0) < 0 ? "font-semibold text-red-600" : "text-stone-600")}>{money(row.margin)}</td>
            <td className={"whitespace-nowrap px-3 py-1.5 text-right tabular-nums " + ((row.sellPrice ?? 0) < (row.suggestedPrice ?? 0) ? "font-semibold text-coral-700" : "text-stone-400")}>{row.suggestedPrice == null ? "—" : money(Math.ceil(row.suggestedPrice / 10) * 10)}</td>
            <td className="whitespace-nowrap px-3 py-1.5"><Badge tone={tone}>{row.flags.length ? "needs data" : (row.foodCostPct ?? 0) > .75 ? "critical" : (row.foodCostPct ?? 0) > .55 ? "high" : "ok"}</Badge></td>
          </tr>
        })}</tbody></table>
      </div>
    </section>
