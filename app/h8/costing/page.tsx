...
export default async function CostingPage() {
  ...
  const kpis: Kpi[] = [
    ...
    { label: "Sales-weighted FC", value: pct(data.kpis.weightedFoodCostPct), sub: money(data.kpis.totalCogs, { compact: true }) + " theoretical COGS", tone: (data.kpis.weightedFoodCostPct ?? 0) > .55 ? "bad" : "default" },
      ...
      <div className="overflow-x-auto rounded-xl border border-stone-200 bg-white shadow-[0_1px_2px_rgba(28,25,23,0.04)]">
        <table className="w-full text-[13px]"><thead><tr className="border-b border-stone-200 bg-stone-50/80 text-[10.5px] uppercase tracking-wide text-stone-400">
          ...
          return <tr key={row.id} className="hover:bg-coral-50/60">
            ...
            <td className="px-3 py-2 text-right tabular-nums text-stone-500">{num(row.unitsSold)}</td><td className="px-3 py-2 text-right tabular-nums text-stone-600">{money(row.theoreticalCogs)}</td>
