...
export default async function UsagePage({ searchParams }: { searchParams: { from?: string | string[]; to?: string | string[] } }) {
  ...
  return <>
    ...
    <form method="get" className="editor-card mb-5 flex flex-wrap items-end gap-4 p-4"><div className="mr-2 min-w-48"><div className="text-[10px] font-semibold uppercase tracking-[0.14em] text-coral-700">Usage window</div><div className="mt-1 text-sm font-semibold text-stone-800">Choose the POS sales period</div><p className="mt-0.5 text-[10.5px] text-stone-400">Settled sales are exploded into raw-material demand.</p></div><div><label className="form-label">From date</label><input type="date" name="from" defaultValue={from} className="form-control min-w-40" /></div><div><label className="form-label">To date</label><input type="date" name="to" defaultValue={to} className="form-control min-w-40" /></div><button className="btn-primary">Run usage report</button><span className="ml-auto rounded-full bg-stone-100 px-3 py-1.5 text-[10.5px] font-semibold tabular-nums text-stone-500">{from} → {to}</span></form>
    <KpiStrip items={kpis} />
