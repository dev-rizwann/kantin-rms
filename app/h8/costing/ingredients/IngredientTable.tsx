...
import { checkStockUomChange, uomDimension } from "@/lib/uom"
...
export function IngredientTable({ ingredients, canEdit, uoms, allCategories }: { ingredients: IngredientRow[]; canEdit: boolean; uoms: { code: string; name: string }[]; allCategories: CostingCategoryRow[] }) {
  ...
  async function save() {
    ...
    const res = await saveCostingIngredient({ id: draft.id, category: draft.category, packPrice: draft.packPrice, packQty: draft.packQty, note: draft.note ?? "", estimated: draft.estimated, costingActive: draft.costingActive, uomCode: draft.uomCode })
    setBusy(false)
  ...
  return <div>
    ...
    <div className="editor-card overflow-x-auto"><table className="w-full text-[11.5px] leading-tight">
      ...
      <tbody className="divide-y divide-stone-100">{rows.map((row) => {
        ...
        // Restate the pack quantity into the new unit so the cost per unit is
        // preserved (1000 GRAM -> 1 KG). Only possible within one dimension.
        const changeUom = (code: string) => setDraft((d) => {
          if (!d) return d
          const { factor } = checkStockUomChange(d.uomCode, code)
          const packQty = factor != null && d.packQty != null ? Number((d.packQty * factor).toPrecision(12)) : d.packQty
          return { ...d, uomCode: code, packQty }
        })
        const uomChanged = active && value.uomCode !== row.uomCode
        const uomVerdict = uomChanged ? checkStockUomChange(row.uomCode, value.uomCode, Array(row.recipeLineCount).fill(row.uomCode)) : null
        ...
        {active && <tr><td colSpan={8} className="p-0"><div className="border-y border-coral-100 bg-[linear-gradient(135deg,rgba(253,242,239,.78),rgba(255,255,255,.96)_58%,rgba(245,250,236,.55))] p-5">
          ...
          <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-coral-100/70 pt-4"><p className="text-[11px] text-stone-400">Saving updates the live product cost and writes an auditable cost-history record.</p><div className="flex gap-2"><button type="button" onClick={() => { setEditing(null); setDraft(null); setError(null) }} className="btn-secondary">Cancel</button><button type="button" disabled={busy || value.packPrice == null || value.packQty == null || uomVerdict?.ok === false} onClick={save} className="btn-primary min-w-32">{busy ? "Saving…" : "Save pack cost"}</button></div></div>
        </div></td></tr>}
        </Fragment>
      })}</tbody>
    </table></div>
