...
export function IngredientTable({ ingredients, canEdit, uoms }: { ingredients: IngredientRow[]; canEdit: boolean; uoms: { code: string; name: string }[] }) {
  ...
  return <div>
    ...
    <div className="editor-card overflow-x-auto"><table className="w-full text-[12px]">
      ...
      <tbody className="divide-y divide-stone-100">{rows.map((row) => {
        ...
        return <Fragment key={row.id}><tr className={active ? "bg-coral-50/60" : row.costingActive ? "transition-colors hover:bg-coral-50/35" : "bg-stone-50/60 text-stone-400"}>
          <td className="max-w-[280px] px-3 py-1"><div className="flex min-w-0 items-baseline gap-2"><span className="shrink-0 font-semibold text-stone-900">{row.name}</span>{row.note && <span className="truncate text-[10px] text-stone-400" title={row.note}>{row.note}</span>}</div></td>
          <td className="whitespace-nowrap px-3 py-1"><span className="rounded bg-stone-100 px-1.5 py-0.5 text-[10px] font-medium text-stone-600">{row.category}</span></td>
          <td className="whitespace-nowrap px-3 py-1 text-[10px] text-stone-400">{row.kind === "RAW_MATERIAL" ? "Raw" : row.kind === "PACKAGING" ? "Packaging" : row.kind}</td>
          <td className="whitespace-nowrap px-3 py-1 text-right tabular-nums">{row.packPrice == null ? "—" : `Rs ${row.packPrice.toLocaleString("en-PK")}`}</td>
          <td className="whitespace-nowrap px-3 py-1 text-right tabular-nums">{`${row.packQty?.toLocaleString("en-PK") ?? "—"} ${row.uomCode}`}</td>
          <td className="whitespace-nowrap px-3 py-1 text-right font-semibold tabular-nums text-stone-900">{row.unitCost == null ? "—" : `Rs ${row.unitCost.toFixed(4)}`}</td>
          <td className="whitespace-nowrap px-3 py-1"><span className={row.estimated ? "rounded-full bg-amber-50 px-1.5 py-0.5 text-[9.5px] font-semibold uppercase tracking-wide text-amber-700 ring-1 ring-amber-100" : "rounded-full bg-leaf-50 px-1.5 py-0.5 text-[9.5px] font-semibold uppercase tracking-wide text-leaf-700 ring-1 ring-leaf-100"}>{row.estimated ? "Confirm" : "Verified"}</span></td>
          <td className="whitespace-nowrap px-3 py-1 text-right">{canEdit ? <button onClick={() => active ? (setEditing(null), setDraft(null), setError(null)) : (setEditing(row.id), setDraft({ ...row }), setError(null))} className={active ? "rounded-md bg-stone-900 px-2.5 py-1 text-[10.5px] font-semibold text-white" : "rounded-md border border-stone-200 bg-white px-2.5 py-1 text-[10.5px] font-semibold text-stone-600 transition hover:border-coral-200 hover:bg-coral-50 hover:text-coral-700"}>{active ? "Close" : "Edit"}</button> : null}</td>
        </tr>
