...
export function ProductForm({ uoms }: { uoms: Uom[] }) {
  ...
  const input = "w-full px-3 py-2.5 border border-slate-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"

  return (
    <form onSubmit={submit} className="max-w-2xl bg-white border border-slate-200 rounded-lg shadow-sm p-6 space-y-4">
      <div>
        <label className="block text-sm font-medium text-slate-700 mb-1">Name *</label>
        <input className={input} value={name} onChange={(e) => setName(e.target.value)} required placeholder="e.g. Potato" />
      </div>
      <div>
        <label className="block text-sm font-medium text-slate-700 mb-1">Name (Urdu, optional)</label>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">Stock unit *</label>
          ...
          <p className="text-xs text-slate-400 mt-1">The unit you count this product in.</p>
        </div>
      </div>
      <div>
        <label className="block text-sm font-medium text-slate-700 mb-1">Min stock (alert below this)</label>
      ...
      <div className="border-t border-slate-100 pt-4">
        <div className="flex items-center justify-between mb-2">
          <label className="text-sm font-medium text-slate-700">Purchase pack sizes (optional)</label>
          <button type="button" onClick={addPack} className="text-xs text-blue-600 hover:underline">+ Add pack</button>
        </div>
        ...
        {packs.map((pk, i) => (
          <div key={i} className="flex items-center gap-2 mb-2">
            ...
            <input
              className="w-28 px-2 py-2 border border-slate-300 rounded-md text-sm"
            ...
            />
            <span className="text-sm text-slate-500">{stockUomCode}</span>
      ...
      <div className="flex gap-3">
        <button type="submit" disabled={busy} className="px-4 py-2.5 bg-blue-600 text-white font-medium rounded-md hover:bg-blue-700 disabled:bg-blue-400">
