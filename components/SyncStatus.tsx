...
import { agoLabel, pktDateTime, posDateTime } from "@/lib/format"
...
export function SyncStatus({ status }: { status: Status }) {
  ...
  return (
    <div className="flex items-center gap-1.5 px-1.5 pb-1.5 pt-0.5 text-[9.5px] leading-tight text-white/45" title={at ? `Last push received ${pktDateTime(at)} PKT${status.lastRowsAt && status.lastRowsAt !== at ? ` · last new rows ${pktDateTime(status.lastRowsAt)} PKT` : ""}` : status.unavailable ? "Could not read the sync state just now." : "No push has ever been received from the canteen PC."}>
      ...
      <span className="sidebar-copy min-w-0 truncate">
        <span className="block truncate">{at ? <>Synced {pktDateTime(at)}{now && <span className="text-white/30"> · {agoLabel(at, now)}</span>}</> : status.unavailable ? "Sync status unavailable" : "Never synced"}</span>
        {status.lastTicketAt && <span className="block truncate text-white/30">Last sale {posDateTime(status.lastTicketAt)}</span>}
      </span>
    </div>
  )
}
