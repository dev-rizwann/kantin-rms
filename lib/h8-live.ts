 ...
 * on the standalone runtime. Heavy views are scanned once via CTEs.
 */
...
export async function getH8OverviewLive(): Promise<H8OverviewLive> {
  const rows = await prisma.$queryRaw<{ payload: any }[]>`
    ...
    lastday AS (SELECT MAX(d) AS d FROM co WHERE NOT void),
    prevday AS (SELECT MAX(d) AS d FROM co WHERE NOT void AND d < (SELECT d FROM lastday))
    SELECT json_build_object(
      ...
      'items_sold', (SELECT COUNT(*) FROM mp_itemsale s WHERE s.kantin_slug=${K}
      ...
      'total_cancel_amount', (SELECT COALESCE(SUM(s.price),0) FROM mp_cancel cn JOIN mp_itemsale s ON s.id=cn.itemsale_id AND s.kantin_slug=${K} WHERE cn.kantin_slug=${K}),
      'total_refunds', (SELECT COUNT(*) FROM mp_refund WHERE kantin_slug=${K}),
      'total_refund_amount', (SELECT COALESCE(SUM(s.price),0) FROM mp_refund r JOIN mp_itemsale s ON s.id=r.itemsale_id AND s.kantin_slug=${K} WHERE r.kantin_slug=${K}),
      ...
      'hourly', (SELECT COALESCE(json_agg(h ORDER BY h.hour_of_day),'[]'::json) FROM (
          ...
          WHERE s.kantin_slug=${K} AND cn.id IS NULL GROUP BY 1 ORDER BY 1) h),
      ...
      'top_items_30d', (SELECT COALESCE(json_agg(ti ORDER BY ti.sales DESC),'[]'::json) FROM (
          SELECT i.id AS item_id, i.title AS item, cat.title AS category, COUNT(*) AS qty, COALESCE(SUM(s.price),0) AS sales
          FROM mp_itemsale s
          JOIN mp_item i ON i.id=s.item_id AND i.kantin_slug=${K}
...
export async function getH8LandingSummary(): Promise<H8LandingSummary> {
  const rows = await prisma.$queryRaw<{ payload: any }[]>`
    WITH co AS (SELECT total, void, created::date AS d FROM mp_checkout WHERE kantin_slug=${K})
    SELECT json_build_object(
